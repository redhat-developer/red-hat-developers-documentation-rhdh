:_mod-docs-content-type: ASSEMBLY
:_mod-docs-category: Explore

include::artifacts/attributes.adoc[]
:title: {explore_preview_of_emerging_and_supplemental_capabilities-book-title}
:context: preview-of-emerging-capabilities
:subtitle: Explore features that are new, evolving, or not yet fully covered in the core product documentation
:abstract: Explore emerging, evolving, and supplemental capabilities in {product}, including new features and supported functionality not yet fully covered in the core documentation.
:imagesdir: images

[id="{context}"]
= {title}

[role="_abstract"]
Explore emerging, evolving, and supplemental capabilities in {product}. This content covers new features, preview technologies, and supported functionality not yet fully documented in the core documentation.

include::additional-capabilities/ref-rhdh-capabilities.adoc[]

include::assemblies/explore_preview-of-emerging-capabilities/assembly-register-and-discover-ai-content-assets-with-the-aicontext-entity.adoc[leveloffset=+1]