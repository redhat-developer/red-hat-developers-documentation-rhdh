...
{product} {product-version} is now generally available from the {company-name} link:https://access.redhat.com/downloads[Customer Portal] or from the link:https://catalog.redhat.com/search?gs&q=rhdh&searchType=containers[Ecosystem Catalog].

{product-short} is a fully supported, enterprise-grade productized version of upstream *{product-custom-resource-type} {product-backstage-version}*. 
