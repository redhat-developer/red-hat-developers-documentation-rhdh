...
{product} ({product-short}) {product-version} is now generally available. {product-short} is a fully supported, enterprise-grade productized version of upstream Backstage, compatible with version {product-backstage-version}. Plugins might be compatible with a newer Backstage version. You can access and download the {product} application from the {company-name} https://access.redhat.com/downloads[Customer Portal] or from the https://catalog.redhat.com/en/search?gs&q=rhdh&searchType=containers[Ecosystem Catalog].
