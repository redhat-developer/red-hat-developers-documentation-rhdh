...
[role="_abstract"]
Internal developer platforms (IDPs) provide a unified interface that enables developer self-service for provisioning environments, deploying code, and accessing APIs.
By centralizing these tools, IDPs reduce complexity across development workflows.
...
Access development tools, CI/CD pipelines, APIs, monitoring tools, and documentation from a single interface. Integrate with systems such as Git, {ocp-brand-name}, Kubernetes, and Jira.
...
* xref:platform-integrations-for-toolchain-connectivity_evaluate-rhdh-capabilities[Integrations in {product}]
