...
== Integrate ArgoCD notifications with {product}

This guide explains how to configure ArgoCD to send notifications directly to the {product} ({product-very-short}) Notifications plugin when application sync events occur.

[IMPORTANT]
====
*Administrative Requirements*

This integration requires infrastructure-level access. To proceed, you must have:
...
The following diagram illustrates the flow from an ArgoCD event in the cluster to the {product-very-short} user interface.
----
┌────────────────────────────────────────────────────────────────────┐
...
* Administrative access to the OpenShift cluster where ArgoCD and {product-very-short} are running.
* ArgoCD installed with the notifications controller enabled.
* Network connectivity from the ArgoCD namespace to the {product-very-short} endpoint.
...
To allow ArgoCD to authenticate with the {product-very-short} backend, you must provide a secure token.
...
Once {product-very-short} is configured to receive notifications, configure the ArgoCD notifications controller to send them.
...
>
> For a complete list of available trigger conditions and variables, see the link:https://argo-cd.readthedocs.io/en/stable/operator-manual/notifications/triggers/[ArgoCD Notifications Triggers documentation].
====
...
*Environment*: OpenShift Cluster (ArgoCD Application manifests).

Update individual ArgoCD `Application` manifests to enable the notifications for specific components.
...
* link:https://argo-cd.readthedocs.io/en/stable/operator-manual/notifications/[ArgoCD Notifications Documentation]
* link:https://backstage.io/docs/notifications/[Backstage Notifications Plugin]
* link:https://argo-cd.readthedocs.io/en/stable/operator-manual/notifications/services/webhook/[ArgoCD Webhook Service Configuration]