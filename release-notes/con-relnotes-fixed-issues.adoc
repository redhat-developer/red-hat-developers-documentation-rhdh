...
In earlier versions, there was no option to configure image pull secrets for a PostgreSQL pod when installing {product} using the Operator.
+
...
+
In {product} 1.1.2, this is resolved and you can configure the image pull secrets for both the {product-short} and PostgreSQL pods in the `spec.application.imagePullSecrets` field.
