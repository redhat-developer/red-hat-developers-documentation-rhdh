...
The Keycloak backend plugin integrates Keycloak into {product-short}.
...
=== Installing and configuring the Keycloak backend plugin
...
Communication between {product-short} and Keycloak is enabled by using the Keycloak API. Username/password or client credentials are supported authentication methods.
...
=== Imported users and groups in {product-short} using Keycloak plugin
