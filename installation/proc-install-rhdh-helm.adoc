...
. Click *Create*.
. Copy the OpenShift router host to *Root Schema* -> *global* -> *Shorthand for users who do not want to specify a custom HOSTNAME. Used ONLY with the DEFAULT upstream.backstage.appConfig value and with OCP Route enabled.*, and adjust the other values if needed.

. Alternatively, copy the OpenShift router host to `global.clusterRouterBase`, and adjust other values if needed, such as `global.clusterRouterBase: apps.example.com`. 
+
--
