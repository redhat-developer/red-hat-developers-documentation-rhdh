...
[role="_abstract"]
Dynamic plugins that you configure for the {backstage} custom resource (CR) might require certain Kubernetes resources to make the plugin work. The documentation refers to these resources as `plugin dependencies`. In {product} ({product-very-short}), you can automatically create these resources when you apply the {backstage} CR to the cluster.
