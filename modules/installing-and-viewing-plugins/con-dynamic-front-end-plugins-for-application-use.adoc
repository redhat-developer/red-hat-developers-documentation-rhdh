...
Front-end wiring configuration occurs in the `{my-app-config-file}` or a dedicated `dynamic-plugins-config.yaml` file. The dynamic plugin exposes components, routes, or APIs. For example, a module (for example, `PluginRoot`) exports a plugin component (`FooPluginPage`).
