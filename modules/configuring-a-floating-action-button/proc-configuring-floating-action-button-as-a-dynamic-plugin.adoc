...
* To configure a dynamic plugin as a floating action button that opens an external link, specify the `global.floatingactionbutton/config` mount point of your `app-config-dynamic.yaml` file in the `backstage-plugin-global-floating-action-button` plugin. For example:
+
...
* To configure a floating action buttons that contains a submenu, define the `global.floatingactionbutton/config` mount point in the same `slot` field of your `app-config-dynamic.yaml` file for multiple dynamic plugins as actions. The default slot is `page-end` when not specified. For example:
+
