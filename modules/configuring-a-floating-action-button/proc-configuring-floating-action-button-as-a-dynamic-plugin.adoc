...
* Specify the `global.floatingactionbutton/config` mount point in your `{my-app-config-file}` file. For example: 
+
...
----
You can also configure floating action buttons to display as submenu options within the main floating action button by assigning the floating action buttons to the same slot.
...
.Example of a floating action button as a submenu using the default `slot` value
[source,yaml]
----
  - package: ./dynamic-plugins/dist/red-hat-developer-hub-backstage-plugin-global-floating-action-button
    disabled: false
