...
<2> Use this `svg` value to display a black BulkImportPage icon.

* To configure a dynamic plugin as a floating action button that opens an external link, specify the `global.floatingactionbutton/config` mount point in your dynamic plugins config map within the `backstage-plugin-global-floating-action-button` plugin. For example:
+
...
----
<1> (Required) The import name with an associated component to the mount point.
<2> Use this `svg` value to display the `Quay` icon.

* To configure a floating action button that contains a submenu, define the `global.floatingactionbutton/config` mount point in the same `slot` field in your dynamic plugins config map for multiple dynamic plugins as actions. The default slot is `page-end` when not specified. For example:
+
...
[NOTE]
====
If multiple floating button actions are assigned to the same `slot` value, these floating buttons are displayed as submenu options within the main floating action button.
====