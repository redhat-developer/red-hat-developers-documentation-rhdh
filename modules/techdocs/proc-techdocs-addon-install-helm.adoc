...
.Procedure
. In your Helm chart, add the `global.dynamic` parameters required to install a dynamic plugin, as shown in {installing-and-viewing-plugins-book-url}#con-install-dynamic-plugin-helm_rhdh-installing-rhdh-plugins[Installing dynamic plugins using the Helm chart ]
+
[NOTE]
====
