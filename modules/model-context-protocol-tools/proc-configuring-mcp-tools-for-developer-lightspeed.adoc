...
.Procedure
. Configure the Llama Stack tool definition by defining the MCP provider in the `tool_runtime` section of the Llama Stack configuration file (`run.yaml`), which is located in the `/app-root-config/llama-stack/' directory shown in the following code:
+
[source,yaml]
----
providers:
...
. Configure the {lcs-short} MCP server endpoint by defining the MCP server endpoints in the {lcs-short} configuration file (`lightspeed-stack.yaml`), which is located in the `/app-root/config/` directory, as shown in the following code:
+
[source,yaml]
----
mcp_servers:
...
. Configure the {ls-brand-name} plugin authorization token by specifying the MCP servers and providing the token for authentication in the {ls-brand-name} plugin configuration file (`lightspeed-app-config.yaml`), which is located in the `/app-root/config/` directory, as shown in the following code:
+
[source,yaml]
----
lightspeed:
