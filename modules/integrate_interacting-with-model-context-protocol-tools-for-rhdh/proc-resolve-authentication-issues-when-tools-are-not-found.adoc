...
If an MCP client connects to the server but cannot find deployed tools, you must verify the authentication status and endpoint resolution.

.Procedure

. Check the token validation status in the {ls-brand-name} interface:
.. In the {ls-brand-name} chat box, click the menu icon and select *MCP settings*.
...
.. Confirm that the configuration for the static token and MCP plugin sources is nested under an existing `backend` field, if one is present. For a reference configuration, see {model-context-protocol-book-link}#configure-model-context-protocol-in-red-hat-developer-hub[Configure MCP in {product-very-shrot}].