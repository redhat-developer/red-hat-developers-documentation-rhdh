...
You must configure an MCP client before it can interact with the MCP server. For a list of supported clients and their specific configurations, see https://modelcontextprotocol.io/clients[Example Clients].
...
.. Optional: To use a custom Llama Stack configuration, add the following code to your Llama Stack configuration file (`run.yaml`).
+
[source,yaml]
----
providers:
...
.. To authorize requests to the MCP endpoint using `<MCP_TOKEN>`, add one or more servers to the `mcpServers` list in the {ls-short} `{my-app-config-file}` file, to make POST requests to {lcs-short}:
+
[source,yaml]
----
lightspeed:
  ...
  - name: _<mcp_server_name>_
    token: ${_<mcp_server_token>_}
----
where:

`name`:: Enter the server name. This must match the name configured in the {lcs-short}.
`token`:: Enter the static token used to authorize requests.
