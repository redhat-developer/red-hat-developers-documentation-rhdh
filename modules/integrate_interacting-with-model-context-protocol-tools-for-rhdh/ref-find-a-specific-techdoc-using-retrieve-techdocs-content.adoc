...
[id="find-a-specific-techdoc-using-retrieve-techdocs-content_{context}"]
= Find a specific TechDoc using `retrieve-techdocs-content`
...
[NOTE]
====
If TechDocs indexing is empty or not yet configured, `retrieve-techdocs-content` returns no results. Use `fetch-techdocs` first to confirm that entities with TechDocs are available before calling this tool.
====
