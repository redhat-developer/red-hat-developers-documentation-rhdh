...
[id="ref-software-template-yaml-file-structure_{context}"]
= Software Template YAML file structure
...
`spec.output`:: Use the `output` section to specify the structure of output data that is created when the template is used. The `output` section, particularly the `links` subsection, provides valuable references and URLs that users can use to access and interact with components that are created from the template.
