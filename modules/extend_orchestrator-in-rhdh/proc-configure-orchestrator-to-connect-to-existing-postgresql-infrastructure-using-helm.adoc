...
By default, when you enable the Orchestrator plugin using the Helm Chart with `orchestrator.enabled=true`, the chart automatically provisions a `SonataFlowPlatform` custom resource (CR) and creates the required PostgreSQL database resources. The chart uses the `orchestrator.sonataflowPlatform` values to configure these resources.
...
.Procedure
include::../shared/snip-create-orchestrator-database.adoc[leveloffset=+1]
...
orchestrator:
  ...
  sonataflowPlatform:
    ...
    externalDBHost: __<SERVICE-NAME-TO-DB>__  # Typically external-postgresql-service
    externalDBPort: "5432"
----
+
Where:
