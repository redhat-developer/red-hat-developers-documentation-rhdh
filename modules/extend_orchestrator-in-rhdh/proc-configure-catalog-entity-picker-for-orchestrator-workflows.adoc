...
* You have an {ocp-short} cluster with the Orchestrator plugin enabled in {product-short}.
...
+
The picker lists only catalog entities that the user is authorized to view. If a required entity is not listed, verify the user's catalog permissions and the entity's visibility.
+
...
[NOTE]
====
For an Orchestrator workflow template, the *Choose* button is disabled when the linked workflow is unavailable. Hover over the *Choose* button to display a tooltip that explains why you cannot run the workflow.
====
