...
|`busy`
|`boolean`
|A boolean value that indicates whether a workflow run is in progress. Disable action buttons when this value is `true` to prevent duplicate submissions.

|`schema`
|`JSONSchema7`
|The JSON Schema definition for the workflow form. Use it for field structure, titles, and UI hints such as hidden fields.

|`data`
|`JsonObject`
|The form data submitted by the user, awaiting review before the workflow runs. This object has all form values structured according to the schema.

|`handleBack`
|`() => void`
|Callback to return to the previous wizard step (same behavior as the default review page). Call it when the user chooses *Back*.

|`handleExecute`
|`() => void`
|A callback function to run the workflow with the reviewed data. Call this function when the user clicks *Run* to start the workflow.
|===
