...
+
Keep historical data::: {product-very-short} retains past workflow execution records, allowing you to view the execution history and results of completed workflow instances.
+
Remove historical data::: Contact your system administrator or Red Hat support for guidance on safely removing historical duplicate entries from the Data Index without affecting active workflow operations.
+
[IMPORTANT]
====
...
* Implement workflow versioning best practices for future deployments.
* Set up monitoring to detect duplicate workflow IDs during deployment.

[role="_additional-resources"]
.Additional resources

* xref:unique-workflow-id-requirements-to-prevent-duplicates_{context}[Unique workflow ID requirements to prevent duplicates]
