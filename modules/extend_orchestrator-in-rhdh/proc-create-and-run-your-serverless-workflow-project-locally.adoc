...
[id="create-and-run-your-serverless-workflow-project-locally_{context}"]
= Create and run your serverless workflow project locally
...
. The `kn-workflow quarkus create` command generates the workflow file in JSON format (`workflow.sw.json`). Convert this file to YAML by using the `yq` tool and remove the original JSON file before building, as shown in the following example:
+
[source,bash]
----
...
----
registry.redhat.io/openshift-serverless-1/logic-swf-devmode-rhel9:{rhoserverless-logic-version}
----
...
[source,yaml,subs=+attributes,+quotes]
----
registry.redhat.io/openshift-serverless-1/logic-swf-builder-rhel9:{rhoserverless-logic-version}
