...
[id="upgrade-the-openshift-serverless-logic-operator-for-rhdh-1-9_{context}"]
= Upgrade the OpenShift Serverless Logic Operator for {product} 1.9

[role="_abstract"]
Upgrade the OpenShift Serverless Logic (OSL) Operator to version {rhoserverless-logic-version} for compatibility with {product} {product-version}.

See {rhoserverless-link}#serverless-logic-upgrading-operator[OpenShift Serverless Logic Operator documentation] for the supported OpenShift Serverless Logic Operator versions and upgrade process.
...
spec:
  ...
  startingCSV: logic-operator.v{rhoserverless-logic-version}  # The initial version of the operator
----
  ...
  podTemplate:
    container:
      env:
        ...
        - name: QUARKUS_DA{rhtas-very-short}OURCE_REACTIVE_POSTGRESQL_SSL_MODE
          value: allow
        - name: QUARKUS_DA{rhtas-very-short}OURCE_REACTIVE_TRUST_ALL
          value: 'true'
----