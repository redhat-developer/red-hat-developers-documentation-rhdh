...
[id="install-the-orchestrator-software-templates-chart_{context}"]
...
----
{product-very-short}_ROUTE="https://$(oc get route -n {{ .Values.orchestratorTemplates.rhdhChartNamespace }} -o jsonpath='{.items[0].spec.host}')"
----
...
sed -i "s|__{product-very-short}_BASE_URL__|${product-very-short}_ROUTE|g" orchestrator-templates-values.yaml
----
