...
[id="upgrade-the-openshift-serverless-logic-operator-for-red-hat-developer-hub-1-9_{context}"]
= Upgrade the OpenShift Serverless Logic Operator for {product} 1.9

[role="_abstract"]
Upgrade the OpenShift Serverless Logic (OSL) Operator to version {rhoserverless-version} for compatibility with {product} {product-version}.
