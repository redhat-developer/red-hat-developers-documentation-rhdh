...
[id="install-rhdh-on-ocp-with-the-orchestrator-using-the-helm-cli_{context}"]
= Install {product} ({product-very-short}) on {ocp-short} with the Orchestrator using the Helm CLI
...
. (Optional) Enable *Notifications* and *Signals* plugins by adding them to the `dynamicPlugins.plugins` list in your `values.yaml` file as shown in the following example:
+
[source,yaml]
----
dynamicPlugins:
