...
[id="install-rhdh-with-orchestrator-in-a-partially-disconnected-ocp-environment-using-the-helm-chart_{context}"]
= Install {product} with Orchestrator in a partially disconnected {ocp-short} environment using the Helm chart
    ...
    - catalog: registry.redhat.io/redhat/redhat-operator-index:v<ocp-version>
     # For example: registry.redhat.io/redhat/redhat-operator-index:v{ocp-version}
      ...
      - name: logic-operator
        channels:
        - name: stable
          minVersion: {rhoserverless-logic-version}
          maxVersion: {rhoserverless-logic-version}
