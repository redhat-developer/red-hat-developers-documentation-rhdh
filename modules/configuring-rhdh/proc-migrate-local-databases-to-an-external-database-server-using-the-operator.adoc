...
to_host=<db-service-host> # Destination host name, for example, <db-instance-name>.rds.amazonaws.com
to_port=5432 # Destination port, such as 5432
to_user=postgres # Destination server username, for example, postgres

from_host=127.0.0.1 # Source host name, such as 127.0.0.1
from_port=15432 # Source port number, such as the <forward-to-port> variable
from_user=postgres # Source server username, for example, postgres
...
----
+
Replace `<destination-db-password>` with the password to connect to the destination database.
+
[NOTE]
====
