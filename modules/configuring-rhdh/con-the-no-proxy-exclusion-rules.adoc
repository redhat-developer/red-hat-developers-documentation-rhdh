...
[id="the-no-proxy-exclusion-rules_{context}"]
= The `NO_PROXY` exclusion rules

[role="_abstract"]
Configure `NO_PROXY` to bypass the proxy for specific hostnames, IP addresses, and port numbers when using {product-short}.
