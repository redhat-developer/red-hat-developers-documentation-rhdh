...
`<ca-certificate-key>`:: The CA certificate key.
`<tls-private-key>`:: Optional: The TLS private key.
`<tls-certificate-key>`:: Optional: The TLS certificate key.
...
stringData:
 ...
 PGSSLMODE: <ssl-mode>
 NODE_EXTRA_CA_CERTS: <abs-path-to-pem-file>
EOF
----
+
Where:
...
`<db-port>`:: Your PostgreSQL instance port number, such as `5432`.
`<username>`:: The user name to connect to your PostgreSQL instance.
`<db-host>`:: Your PostgreSQL instance DNS or IP address.
`<ssl-mode>`:: Optional: For TLS connections, the required SSL mode.
`<abs-path-to-pem-file>`:: Optional: For TLS connections, the absolute path to the PEM file, for example `/opt/app-root/src/postgres-crt.pem`.
...
`{my-product-database-certificates-secrets}`:: The certificate secret name, required if TLS connection is configured.
