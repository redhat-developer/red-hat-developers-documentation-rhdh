...
[id="upgrade-the-red-hat-developer-hub-operator_{context}"]
= Upgrade the {product} Operator
...
* You have configured the appropriate roles and permissions within your project to create or access an application. For more information, see the link:{ocp-docs-link}/html-single/building_applications/index#building-applications-overview[Building applications] in the {ocp-brand-name} documentation.
...
[IMPORTANT]
====
If you are on the Orchestrator plugin `1.7`, you must manually update the plugin configuration after approval to avoid a failed deployment. For more information, see {orchestrator-book-link}#proc-upgrading-the-orchestrator-plugin_install-rhdh-orchestrator[Upgrading the Orchestrator plugin from `1.7` to {product-version}].
====
