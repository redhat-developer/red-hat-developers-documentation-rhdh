...
[IMPORTANT]
====
If you are on the Orchestrator plugin `1.7`, you must manually update the plugin configuration after approval to avoid a failed deployment. For more information, see {orchestrator-book-link}#proc-upgrading-the-orchestrator-plugin[Upgrading the Orchestrator plugin from `1.7` to {product-version}].
====
