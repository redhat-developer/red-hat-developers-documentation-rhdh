...
. Reconfigure your `{product-custom-resource-type}` custom resource (CR). For more information, see {configuring-book-link}#proc-configuring-postgresql-instance-using-operator_configuring-external-postgresql-databases[Configuring an external PostgreSQL instance using the Operator].
