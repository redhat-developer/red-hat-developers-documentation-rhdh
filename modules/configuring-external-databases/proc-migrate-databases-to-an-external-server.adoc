...
[id="migrate-databases-to-an-external-server_{context}"]
= Migrate local databases to an external database server using the Operator

[role="_abstract"]
Migrate data from a local PostgreSQL server to an external PostgreSQL service using PostgreSQL utilities such as `pg_dump` and `psql`.
