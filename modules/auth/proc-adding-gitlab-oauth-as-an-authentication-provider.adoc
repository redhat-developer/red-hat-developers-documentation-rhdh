...
<4> Optionallly, when using a custom redirect URI: uncomment, and enter the URL matching the *Redirect URI* registered when creating your GitLab OAuth App, such as:
* For local development: `pass:[http://localhost:7007/api/auth/gitlab/handler/frame]`
* For a non-local deployment: `pass:c,a,q[http://_<APP_FQDN>_/api/auth/gitlab/handler/frame]`
+
NOTE: Due to a peculiarity with GitLab OAuth, ensure the URL has no trailing `/` after 'frame'.
