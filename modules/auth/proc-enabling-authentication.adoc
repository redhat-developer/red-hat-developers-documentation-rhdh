[id='assembly-enabling-authentication_{context}']
= Enabling authentication in {product}

Authentication within the {product} facilitates user sign-in, identification, and access to external resources.
It supports multiple authentication providers.
Usually, set up:

- One provider for sign-in and identification.
- Additional providers for accessing external resources.

The {product} supports the following authentication providers:

Auth0:: `auth0`
Atlassian:: `atlassian`
Microsoft Azure:: `microsoft`
Microsoft Azure Easy Auth:: `azure-easyauth`
Bitbucket:: `bitbucket`
Bitbucket Server:: `bitbucketServer`
Cloudflare Access:: `cfaccess`
GitHub:: `github`
GitLab:: `gitlab`
Google:: `google`
Google IAP:: `gcp-iap`
OIDC:: `oidc`
Okta:: `okta`
OAuth 2 Custom Proxy:: `oauth2Proxy`
OneLogin:: `onelogin`
SAML:: `saml`

.Procedure

For each provider, follow the dedicated procedure to:

. Set up the required secrets in your provider.
. Set up the authentication provider in the {product} backend (`app-config.yaml`).
. (Optionally) To enable the provider for sign-in, set up the provider in the {product} front-end (`packages/app/src/app.tsx`).
