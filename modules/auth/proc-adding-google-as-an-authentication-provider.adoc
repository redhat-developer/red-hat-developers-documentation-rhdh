...
. Set up the Google authentication provider in the {product} configuration (`app-config.yaml`).
+
...
<3> `signInPage` (optional):: Enter `google` to enable the Google provider for sign-in.
