...
{configuring-book-link}#provisioning-your-custom-configuration[{product-very-short} secrets]. You can use these secrets in the {product-very-short} configuration files by using the corresponding environment variable name for each secret.
+
...
`scorecard:metricProviders:github:openPRs:thresholds`:: Lists the default threshold values for the GitHub open pull requests (PRs) metric.
+
[NOTE]
====
