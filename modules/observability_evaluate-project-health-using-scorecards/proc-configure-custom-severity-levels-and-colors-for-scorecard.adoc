...
[NOTE]
====
Each custom severity threshold key requires both a corresponding icon configuration value and a color parameter configuration value. Because the scorecard plugin validates the YAML configuration files during application startup, omitting either property or providing an invalid value causes an initialization failure, and the Scorecard plugin fails to function.
====
