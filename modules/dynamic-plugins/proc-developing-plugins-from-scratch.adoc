...
[NOTE]
====
You can also use {product-very-short} Local to test your plugins by copying the `dist-dynamic/` folder contents to the {product-very-short} Local `local-plugins` folder or by using an exported dynamic plugin package from your container registry. For more details, see _Verify plugins locally_.
====
