...
= Exporting {plugin-type-name} plugins in {product}
...
You should use this command when you have the source code for a custom plugin and want to integrate it into {product-very-short} as a dynamic plugin.
...
For an example of using this command, see xref:assembly-package-publish-third-party-dynamic-plugin[Example of installing a custom plugin in Red Hat Developer Hub].
====
...
+
--
.Example command to export a {plugin-type-name} plugin
[source,terminal]
----
npx @red-hat-developer-hub/cli@latest plugin export
----
