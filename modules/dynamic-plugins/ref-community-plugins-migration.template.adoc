...
* Referenced using local paths, such as `./dynamic-plugins/dist/<plugin-name>`.
...
* Referenced by using OCI registry paths, such as `oci://ghcr.io/redhat-developer/rhdh-plugin-export-overlays/<plugin-name>:<tag>`, or ideally by a SHA digest.
...
[NOTE]
====
Replace `<tag>` with the version tag corresponding to your {product-short} version. For the latest tags, see link:https://github.com/redhat-developer/rhdh-plugin-export-overlays[rhdh-plugin-export-overlays repository].
