...
spec:
 ...
 database:
   enableLocalDb: true
----
... Update your `dynamic-plugins-rhdh` config map to include your marketplace configuration file, as follows:
+
[source,yaml]
----
kind: ConfigMap
...
... Upgrade the Helm release to include your marketplace configuration file and update the `NODE_ENV` environment variable  to `development`:
+
[source,yaml]
----
global:
