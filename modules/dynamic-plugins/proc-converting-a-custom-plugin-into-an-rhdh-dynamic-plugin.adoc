...
Container Image Publishing: Optionally push to container registries (Quay, OpenShift, etc.)The Dynamic Plug-in Factory is an experimental Developer Preview tool from the Red Hat team that shifts the plug-in build process from an imperative approach to a declarative one. Instead of running a sequence of commands, you define build parameters with configuration files.
...
Prerequisites
You only need a container engine like Podman or Docker installed in your environment.
Once you have installed a container engine, pull the Developer Hub compatible version of the dynamic plugin factory image.
