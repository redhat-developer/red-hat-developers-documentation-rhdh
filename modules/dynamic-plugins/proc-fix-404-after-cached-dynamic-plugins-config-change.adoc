...
In rare cases, making changes to the cached dynamic plugins configuration can lead to a temporary *404* error.
...
. In your PVC configuration file, set `spec.deployment` to use the optional `StatefulSet` as a resource kind as shown in the example below:
+
[,console]
----
deployment:
