...
= Provision your {rhec} pull secret to your {product} instance namespace

On {platform-long}, the {rhec} pull-secret is not managed globally.
...
.Procedure
. Create a pull secret using your {company-name} credentials to pull the container images from the protected {rhec}:
+
...
. To enable pulling {product-short} images from the {rhec}, add the image pull secret in the default service account within the namespace where the {product-short} instance is being deployed:
+
[source,terminal,subs="+attributes"]
----
