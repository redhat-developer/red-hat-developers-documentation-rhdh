...
= Installing {product} on {ocp-short} using the Operator
...
* A cluster administrator has installed the {product} Operator. For more information, see link:{LinkAdminGuide}[Installing the {product} Operator].
...
. Click *Create*.
. Add custom configurations for the {product} instance.
