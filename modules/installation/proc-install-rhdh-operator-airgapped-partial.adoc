...
If you do not already have a target registry, and if you have an {ocp-short} cluster, you might want to expose and leverage the internal cluster registry.
...
* You have an active `oc registry`, `podman`, or `Skopeo` session to the `registry.redhat.io` {rhec}.
