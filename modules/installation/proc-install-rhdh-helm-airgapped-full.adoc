...
----
oc mirror -c <mirror_config_directory>/ImageSetConfiguration.yaml file://<mirror_archive_directory> --v2
----
+
--
where:
...
[source,terminal,subs="+quotes"]
----
oc mirror -c <image-set-config> --from file://<mirrror-archive-directory>
...
----
oc apply -f
<workspace_directory>/working-dir/cluster-resources/idms-oc-mirror.yaml
oc apply -f
<workspace_directory>/working-dir/cluster-resources/itms-oc-mirror.yaml
