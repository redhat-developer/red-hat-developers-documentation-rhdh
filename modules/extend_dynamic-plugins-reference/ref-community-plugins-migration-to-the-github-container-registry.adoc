...
* Add explicit configuration under `pluginConfig:` for each plugin (previously bundled implicitly)
...
* Configured with implicit configuration included in the bundle
...
* Requiring explicit configuration under `pluginConfig:` for each plugin
+
...
.. Find `appConfigExamples[0].content` — this contains the configuration that goes under `pluginConfig:`
...
[NOTE]
====
The exact configuration structure varies by plugin.
