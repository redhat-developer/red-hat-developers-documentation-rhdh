...
+
.Example of creating a new role for the team lead by using the RBAC backend API
[source,bash]
----
...
+
.Example `curl` of applying a conditional access policy by using the `IS_OWNER` rule for the team lead role
[source,bash]
----
