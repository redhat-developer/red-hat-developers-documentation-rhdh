...
[id="inject-custom-files-and-environment-variables-into-backstage-containers_{context}"]
= Inject extra files and environment variables into {backstage} containers

[role="_abstract"]
Inject extra files and environment variables into {backstage} containers by mounting ConfigMaps and Secrets using the `mountPath` field.
