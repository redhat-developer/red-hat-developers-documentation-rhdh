...
[role="_abstract"]
Provision custom config maps and secrets to {platform-long} to configure {product} before running the application.
...
.. Provision your `{my-app-config-file}` file to the `{my-app-config-config-map}` config map and your `dynamic-plugins.yaml` file to the `dynamic-plugins-rhdh` config map in the _<{my-product-namespace}>_ project.
+
[source,terminal,subs="+attributes,+quotes"]
----
