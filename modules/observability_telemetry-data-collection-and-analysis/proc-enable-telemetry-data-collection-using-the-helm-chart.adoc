...
[id="enable-telemetry-data-collection-using-the-helm-chart_{context}"]
= Enable telemetry data collection using the Helm chart
...
.. Select the *Enable the plugin* checkbox.
.. Click *Upgrade*.
...
----
+
*** If you have configured the plugin, search it in your Helm configuration and set the `enabled` property to `true`.
