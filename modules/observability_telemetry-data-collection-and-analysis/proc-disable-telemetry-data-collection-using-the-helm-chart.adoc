...
[id="disable-telemetry-data-collection-using-the-helm-chart_{context}"]
= Disable telemetry data collection using the Helm chart
...
** Using *Form view*
+
.. Expand *Root Schema → Dynamic plugin system configuration → List of dynamic plugins*.
.. Click the *Add list of dynamic plugins* link.
...
+
image::observability_telemetry-data-collection-and-analysis/disabling-telemetry.png["Disabling telemetry data collection in the Helm chart"]
+
