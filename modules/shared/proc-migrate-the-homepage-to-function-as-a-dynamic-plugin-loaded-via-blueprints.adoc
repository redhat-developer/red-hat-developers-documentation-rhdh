...
[id="migrate-the-homepage-to-function-as-a-dynamic-plugin-loaded-via-blueprints_{context}"]
= Migrate the Homepage to function as a dynamic plugin loaded via Blueprints

[role="_abstract"]
Migrate the dynamic homepage to function as a plugin loaded via blueprints to choose which templates and widgets appear when you log in. You can use customizable layouts to drag and resize cards, or fixed layouts with predefined positions.
