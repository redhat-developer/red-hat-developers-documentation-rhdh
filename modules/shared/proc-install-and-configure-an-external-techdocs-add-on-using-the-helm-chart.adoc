...
[id="install-and-configure-an-external-techdocs-add-on-using-the-helm-chart_{context}"]
= Install and configure an external TechDocs add-on using the Helm chart
...
.Procedure
. In your Helm chart, add the `global.dynamic` parameters required to install a dynamic plugin, as shown in {installing-and-viewing-plugins-book-link}#installing-dynamic-plugins-using-the-helm-chart_install-dynamic-plugins-in-rhdh[Installing dynamic plugins using the Helm chart]
+
[NOTE]
====
