...
// TODO (RHIDP-16107): Confirm the exact OKP sidecar container name in the operator bundle
// (replacing the old "init-rag-data" init container). Update <okp-container-name> below.
. Identify the OKP and {lcs-short} sidecar container image tags from the extracted configuration file:
+
[source,terminal]
----
LS_OKP_IMAGE=$(yq '.data["deployment.yaml"]' lightspeed-config.yaml | yq '.spec.template.spec.containers[] | select(.name == "<okp-container-name>") | .image')
