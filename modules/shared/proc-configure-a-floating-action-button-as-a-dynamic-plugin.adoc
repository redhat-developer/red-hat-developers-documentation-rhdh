...
[id="configure-a-floating-action-button-as-a-dynamic-plugin_{context}"]
= Configure a floating action button as a dynamic plugin
...
. To configure translation support for the floating action button, use translation keys instead of static text. The Global Floating Action Button plugin supports internationalization (i18n) through the `labelKey` and `toolTipKey` properties.
+
