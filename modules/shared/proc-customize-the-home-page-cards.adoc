...
[role="_abstract"]
Customize the Home page layout and integrate specialized cards using the 12-column grid system.
For persona-specific layouts, see xref:configure-persona-specific-homepage-layouts-for-role-based-experiences_{context}[].
...
For more information, see xref:install-the-homepage-backend-plugin-to-enable-persona-based-homepages_{context}[Installing the homepage backend plugin].
