...
+
Basic permissions follow the naming pattern `<resource>.<action>`, for example, `catalog.entity.read`.
...
----
if: {
  permissions: { $contains: 'catalog.entity.refresh#update' },
}
----
--
+
[NOTE]
====
The `if` predicate supports only basic permissions. You cannot use resource-based permissions (permissions with a `resourceType`) in the `if` predicate, because they depend on a resource context that is not available when the predicate is evaluated during frontend initialization.
====
