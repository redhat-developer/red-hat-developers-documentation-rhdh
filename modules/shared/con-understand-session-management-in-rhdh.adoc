...
AutoLogout is a frontend-only mechanism: it does not revoke tokens or end Identity Provider (IdP) sessions.
...
When this duration elapses, the session expires even if the user is actively working.
...
== Token expiration ({backstage} tokens)

Token expiration controls the validity period of the short-term cryptographic tokens that {product-short} uses internally.
This is not the same as session duration: tokens are refreshed automatically within an active session.

The default token expiration is 1 hour, and the valid range is 10 minutes to 24 hours.

You configure token expiration per provider by using the `auth.providers.<name>.<env>.backstageTokenExpiration` parameter in your `{my-app-config-file}` file.
...
The four session management mechanisms operate at different layers:
...
Token expiration::
Controls internal token refresh within an active session.
Tokens are refreshed automatically; users are not affected unless the session itself has expired.
