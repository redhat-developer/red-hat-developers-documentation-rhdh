...
** Available only if MCP is configured: Click *MCP and Prompt Settings* to {model-context-protocol-book-link}[manage Model Context Protocol connections].
+
image::shared/lightspeed-chat-mcp.png["Lightspeed MCP and Prompt Settings menu"]
+
[NOTE]
====
Notebooks are also available in overlay, docked, and fullscreen display modes. In overlay and docked modes, the resource panel is togglable.
====
...
** *Rename a session*: Double-click the chat title to edit it inline. Press *Enter* to save the new name or *Escape* to cancel. Alternatively, click *Options* next to a chat title and select *Rename* to activate inline editing.
+
image::shared/lightspeed-chat-rename.png["Inline rename for chat"]
