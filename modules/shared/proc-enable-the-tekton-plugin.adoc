...
[id="enable-the-tekton-plugin_{context}"]
= Enable the Tekton plugin
...
.. If you are using the Tekton plugin that is pre-loaded in {product-very-short}, in addition to setting the `disabled` property to `false`, you must also add the plugin configuration as follows:
+
--
...
.. If you are using the plugin that is packaged as an OCI image, set the `disabled` property to `false` as follows:
+
--
[source,yaml,subs="+quotes"]
----
  ...
  dynamic:
    ...
    plugins:
      - package: oci://ghcr.io/redhat-developer/rhdh-plugin-export-overlays/backstage-community-plugin-tekton:__<tag>__
        disabled: false
----
include::{docdir}/artifacts/snip-tag-for-OCI-package-paths.adoc[leveloffset=+1]
--
