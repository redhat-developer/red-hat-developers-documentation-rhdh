...
[id="configure-entity-tab-titles_{context}"]
= Configure entity tab titles

[role="_abstract"]
{product} provides a default opinionated tab set for catalog entity views. For consistency with your organization needs, you can rename, reorder, remove, and add tab titles.

.Procedure
* For each tab to modify, enter your required values in the `entityTabs` section in your `{my-app-config-file}` file:
+
