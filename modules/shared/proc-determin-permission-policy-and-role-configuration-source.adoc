...
[id="determin-permission-policy-and-role-configuration-source_{context}"]
= Determin permission policy and role configuration source

[role="_abstract"]
Identify the source of permission policies and roles to maintain data consistency and determine which source controls each resource.
...
Configuration file::
Configure roles and policies in the {product-short} `{my-app-config-file}` configuration file, for instance to xref:enable-and-give-access-to-the-role-based-access-control-rbac-feature_{context}[declare your policy administrators].
+
