...
[id="customize-the-branding-logo-of-your-rhdh-instance_{context}"]
= Customize the branding logo of your {product-short} instance

[role="_abstract"]
You can customize the branding logo of your {product-short} instance by configuring the `branding` section in the `{my-app-config-file}` file.

.Procedure

* Customize the branding logo by configuring the `branding` section in the `{my-app-config-file}` file:
+
...
----
+
The following example demonstrates how to customize the `BASE64_EMBEDDED_FULL_LOGO` by using the `data:_<media_type>_;base64,_<base64_data>_` format:
+
[source,yaml,subs="+quotes"]
----
