...
[id="attach-homepages-to-rbac-groups-to-control-access-and-automate-layout-assignment_{context}"]
= Attach homepages to RBAC groups to control access and automate layout assignment

[role="_abstract"]
Attach persona-specific homepage layouts to RBAC groups to control access and automate layout assignment, ensuring users automatically see only the homepage content relevant to their role based on group membership.
...
. Attach persona-specific layouts to RBAC groups by adding the `if.groups` field to group cards under a shared visibility condition.
+
For example, attach the developer persona layout to the developers RBAC group:
+
[source,yaml]
----
homepage:
...
. Control visibility for multiple RBAC groups by adding additional `if.groups` configurations.
+
