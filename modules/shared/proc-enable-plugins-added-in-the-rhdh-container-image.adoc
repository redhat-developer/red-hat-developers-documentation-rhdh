...
[NOTE]
====
You can use the `{{inherit}}` tag if you do not want to manually update your `dynamic-plugins.yaml` file after every release (including minor releases). However, this method is only available for plugins from the plugin catalog index.
====
