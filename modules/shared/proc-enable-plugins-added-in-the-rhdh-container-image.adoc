...
= Enable pre-loaded container plugins
...
. Set the `enabled` field to `true` and add the package name as follows:
+
[NOTE]
====
The `enabled` field requires {product-short} 2.1.0 or later. For earlier versions, use `disabled: false`.
====
+
--
[source,yaml]
...
----
+
include::snip-inherit-tag-rollback-warning.adoc[lines=2..-1]
+
[NOTE]
====
Use OCI URLs (`oci://registry...`) for plugins pulled from container registries.
Use file paths (`./dynamic-plugins/dist/...`) for plugins preloaded in the container image wrapper format.
====
...
For backward compatibility, you can also use the legacy `disabled: false` syntax:
...
. Check the `install-dynamic-plugins` init container logs for successful plugin installation:
+
[source,shell]
----
oc logs <pod-name> -c install-dynamic-plugins | grep "Successfully loaded plugin"
----
. Verify the plugin appears in the {product-short} UI (location varies by plugin type).
