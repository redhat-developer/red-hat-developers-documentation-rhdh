...
[id="configure-entity-detail-tab-layout_{context}"]
= Configure entity detail tab layout

[role="_abstract"]
Each {product} entity detail tab has a default opinionated layout. For consistency with your organization needs, you can change the entity detail tab content when the plugin that contributes the tab content allows a configuration.
...
The example uses 6 of the 12 columns to show two Tekton CI cards side-by-side on large (`lg`) screens (`span 6` columns) and shows them among themselves (`xs` and above `span 12` columns).
