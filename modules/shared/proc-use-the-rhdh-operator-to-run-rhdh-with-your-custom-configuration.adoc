...
[role="_abstract"]
Use the {product} Operator to deploy {product-short} with custom configuration by creating a custom resource that mounts config maps and injects secrets.
...
* You have a working default storage class, such as the Elastic Block Store (EBS) storage add-on, configured in your {eks-short} cluster.
...
----
+
include::snip-warning-single-entry-appconfig-configmap.adoc[]

`spec.application.extraEnvs.envs`:::
Optionally, enter your additional environment variables that are not secrets, such as {configuring-book-link}#configure-proxy-in-operator-deployment_running-behind-a-proxy[your proxy environment variables].
+
...
`spec.deployment`:::
Optionally, {configuring-book-link}#configure-rhdh-deployment-when-using-the-operator_configuring-rhdh[enter your deployment configuration].
