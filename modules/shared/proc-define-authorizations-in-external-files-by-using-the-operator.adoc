...
[id="define-authorizations-in-external-files-by-using-the-operator_{context}"]
= Define authorizations in external files by using the Operator

[role="_abstract"]
Define permissions and roles in external CSV and YAML files and configure {product-short} to use them with the Operator.
...
. Update {configuring-book-link}[your `{product-custom-resource-type}` custom resource] to mount in the {product-short} filesystem your files from the `rbac-policies` config map:
+
