...
[id="quicklinks-and-starred-items-in-the-global-header_{context}"]
...
`MenuItemLink entries`:: Define sections, titles, icons, and links within the *Quicklinks* matrix. The default configuration includes links to the {product-short} documentation and an {product-local-very-short}, as shown in the following configurations:
+
...
- mountPoint: global.header/application-launcher
  ...
  config:
    ...
    props:
      title: {product-short}
      icon: developerHub
      link: <link_to_product_documentation>
...
- mountPoint: global.header/application-launcher
  ...
  config:
    ...
    priority: 100
    props:
      title: "{product-local-very-short}"
      icon: developerHub
      link: <link_to_rhdh_local>
----

[NOTE]
====
When upgrading from earlier versions, installation program does not overwrite your existing `dynamic-plugins.yaml` configuration. If you had not configured *Starred Items* or *Quicklinks* before, they remain disabled after the upgrade and must be manually enabled.
====

[role="_additional-resources"]
.Additional resources
* link:https://docs.redhat.com/en/documentation/red_hat_developer_hub[{product-short} documentation]
* link:https://github.com/redhat-developer/rhdh-local[{product-local-very-short}]
