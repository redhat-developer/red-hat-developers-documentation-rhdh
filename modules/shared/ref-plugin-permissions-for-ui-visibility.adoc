...
[role="_abstract"]
The following table lists the permissions used by various plugins to control UI element visibility. These permissions follow the naming pattern `<resource>.<action>`.

To evaluate a permission together with its `action` attribute in the `if` predicate, append the action by using the `+#action+` suffix, for example, `+kubernetes.clusters.read#read+`.
...
|Bulk Import
|`bulk.import`
|Controls visibility of the Bulk Import page and navigation item
