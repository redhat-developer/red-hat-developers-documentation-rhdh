...
[role="_abstract"]
Configure custom toolbar buttons and dropdown menu items in the {product} global header through your application configuration file to give users one-click access to frequently used tools, documentation, and support resources.

The new front-end system provides three approaches: configuration-driven items through `{my-app-config-file}` without code, extension blueprints for plugin contributions, or fully custom React components. The global header uses `GlobalHeaderComponentBlueprint` for toolbar items and `GlobalHeaderMenuItemBlueprint` for dropdown menu entries.
...
globalHeader:
  ...
  menuItems:
    - target: help  # Target specifies which dropdown menu (`help`, `profile`, or `app-launcher`) receives this item.
      ...
      section: resources  # Section groups related menu items together.
      priority: 80
----
