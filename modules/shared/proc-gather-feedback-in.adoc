...
[id="gather-feedback-in_{context}"]
= Gather feedback in {ls-short}
...
. To enable feedback collection, in the {lcs-short} configuration file (`lightspeed-stack.yaml`), add the following settings:
+
[source,yaml]
----
...
. To disable feedback collection, in the {lcs-short} configuration file (`lightspeed-stack.yaml`), add the following settings:
+
[source,yaml]
----
  user_data_collection:
