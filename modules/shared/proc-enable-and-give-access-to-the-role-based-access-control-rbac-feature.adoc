...
[id="enable-and-give-access-to-the-role-based-access-control-rbac-feature_{context}"]
= Enable and give access to the role-based access control (RBAC) feature
...
The permission policies for users and groups in the {product-short} are managed by permission policy administrators.
...
. To display the available permissions provided by installed plugins in the {product-short} UI, you must supply the corresponding list of plugin IDs.
There are two ways to do this, by updating your application configuration or by using the RBAC REST API permissions endpoint.
+

.. To supply plugins by updating your application configuration, you can specify the plugins with permissions in your `{my-app-config-file}` file as follows:
+
.`{my-app-config-file}` fragment
[source,yaml,subs=+quotes]
----
...
.. To specify the plugins with permissions by using the RBAC REST API permissions endpoint, see the xref:supported-rbac-rest-api-endpoints_manage-authorizations-by-using-the-rest-api[RBAC REST API permissions endpoint].
