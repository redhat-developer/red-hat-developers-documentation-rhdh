...
[id="configure-an-rhdh-development-environment-to-install-plugins-by-using-extensions_{context}"]
= Configure an {product-very-short} Development environment to install plugins by using Extensions
...
... Update your {product-custom-resource-type} CR to update the `NODE_ENV` environment variable  to `development`, as follows:
+
