...
* *Basic permission checks only:* The `if` predicate supports only basic permission checks based on the permission name. You can evaluate the `action` attribute by appending it to the permission name with the `+#action+` suffix, for example, `+catalog.entity.create#create+`.
...
* *Conditional visibility based on entity metadata not supported:* Hiding tabs or other UI elements based on specific entity labels or annotations is not supported. The `if` predicate evaluates permissions only and cannot inspect entity metadata.
