...
`name: dynamic-plugins-npmrc`:: The secret name can be any valid name. You will reference this name in the custom resource configuration.
...
where:
+
`- name: dynamic-plugins-npmrc`:: The secret name must match the secret created in step 1.
