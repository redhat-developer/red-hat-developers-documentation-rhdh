...
[id="match-users-by-ldap-uuid-with-rhbk_{context}"]
= Match users by LDAP UUID with {rhbk-brand-name}
...
For more information, see {authentication-book-link}#enable-user-provisioning-with-ldap_import-users-and-groups-from-your-identity-provider[Enable user provisioning with LDAP].
