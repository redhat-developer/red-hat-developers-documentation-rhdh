...
[IMPORTANT]
====
* {product} {product-version} supports vLLM, native OpenAI, Vertex AI, and Ollama providers. While the vLLM provider uses the OpenAI API schema, it does not support the official OpenAI service (api.openai.com). To use official OpenAI credentials, you must use the native OpenAI provider instead of vLLM. Using OpenAI credentials with the vLLM provider causes errors or improper responses.

* You must declare the safety model environmental variables (`SAFTEY_MODEL`, `SAFETY_URL`, and `SAFETY_API_KEY`). If these are not configured correctly, Llama Stack container runtime errors occur, resulting in user interface failures.
====
