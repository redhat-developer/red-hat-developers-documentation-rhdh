...
data:
  lightspeed-stack.yaml: |
    ...
    service:
      host: 0.0.0.0
    # Use ${env.LIGHTSPEED_SERVICE_PORT} if you are not running the Service on port `8080`
    # port: ${env.LIGHTSPEED_SERVICE_PORT}
      auth_enabled: false
      workers: 1
      color_log: true// Beginning of change. Fixes syntax bug by adding 'env.' prefix to environment variable.
      access_log: true
    llama_stack:
...
[IMPORTANT]
====
{product} {product-version} uses the vLLM provider exclusively. While the vLLM provider supports the OpenAI API schema, it does not support the official OpenAI service (`api.openai.com`). You must not use official OpenAI API URLs or token with vLLM in this release. Using these credentials results in errors or improper responses. Native OpenAI provider support is planned for future releases.
====
...
stringData:
  ...
  VALIDATION_MODEL_NAME: "_<model_name>_"
----
+
where:
...
`VALIDATION_PROVIDER`:: The model provider used for model validation.
`VALIDATION_MODEL_NAME`:: The name of the model used for model validation.
.. Click *Create*.
