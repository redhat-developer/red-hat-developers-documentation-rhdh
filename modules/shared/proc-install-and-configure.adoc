...
[IMPORTANT]
====
* {product} {product-version} supports vLLM, native OpenAI, and Google Vertex AI providers. The vLLM provider uses the OpenAI API schema but it does not support the official OpenAI service (api.openai.com). To use official OpenAI credentials, you must use the native OpenAI provider instead of vLLM. Using OpenAI credentials with the vLLM provider causes errors or improper responses.
====
