...
[IMPORTANT]
====
{lcs-short} uses Llama Stack expansion syntax for environment variables. To make sure the service correctly parses variables in the `lightspeed-stack.yaml` file, you must use the `${env.VAR}` format. Variable names are case-sensitive and must be uppercase. The syntax `${env.var}` is not supported. For example:
[source,yaml]
----
model_config:
  api_key: ${env.VAR}
