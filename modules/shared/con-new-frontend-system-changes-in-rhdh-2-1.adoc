...
Red Hat plugins:: All Red Hat-provided plugins use extension blueprints. You do not need to take any action for these plugins when you install them as dynamic plugins. If you wire a Red Hat plugin directly into a custom `packages/app/src/App.tsx` file, use the plugin's current stable import path instead of its earlier `/alpha` export. See the procedures in this assembly for up-to-date import examples.
