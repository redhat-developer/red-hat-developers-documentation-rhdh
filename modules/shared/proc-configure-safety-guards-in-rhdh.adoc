...
// and /rag-content/ paths below reflect the old FAISS-based RAG setup. With the migration to OKP,
// these sections may need to be replaced with OKP-equivalent configuration or removed entirely
// if OKP auto-configures RAG. Confirm the correct run.yaml schema with engineering before updating.
+
[source,yaml]
----
version: 2
