...
+
// TODO (RHIDP-16107): The vector_io (inline::faiss), FAISS storage backends (sql_files, faiss_kv),
// and /rag-content/ paths below reflect the old FAISS-based RAG setup, which OKP replaces.
// Per engineering, OKP RAG is configured in lightspeed-stack.yaml (rag.tool: - okp; okp block) and is
// auto-configured by the operator and Helm chart, not in run.yaml. Confirm the OKP-equivalent run.yaml
// schema (whether these FAISS sections are removed) with engineering before updating.
+
[source,yaml]
----
version: 2
