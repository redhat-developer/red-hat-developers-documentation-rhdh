...
[id="enable-github-repository-discovery_{context}"]
...
.Procedure
. Create a GitHub App to allow {product-short} to access the GitHub API.
...
`GITHUB_HOST`::
Enter the GitHub hostname without a scheme, such as `github.com`.
The `integrations.github` host expects a hostname only; a URL such as `https://github.com` fails integration validation.
`GITHUB_ORG`::
