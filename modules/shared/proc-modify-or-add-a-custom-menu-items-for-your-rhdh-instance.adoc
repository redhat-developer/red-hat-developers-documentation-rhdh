...
[id="modify-or-add-a-custom-menu-items-for-your-rhdh-instance_{context}"]
= Modify or add a custom menu items for your {product-short} instance

[role="_abstract"]
You can modify existing main menu items or add custom menu items to the sidebar navigation for your {product-short} instance.
...
default.main-menu-items:
      menuItems:
        ...
        default.references:
          ...
          enabled: true
----
`icon`:: (Optional) Enter the icon name, such as `category`, bookmarks`, `school`, and so on to change the default icon.
...
`parent`:: Enter the parent menu under which to nest the menu entry, such as `default.parentlist`.
