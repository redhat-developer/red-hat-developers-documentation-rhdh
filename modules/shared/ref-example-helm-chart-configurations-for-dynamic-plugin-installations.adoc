:_mod-docs-content-type: REFERENCE

[id="example-helm-chart-configurations-for-dynamic-plugin-installations_{context}"]
= Example configurations

[role="_abstract"]
The following examples show how to configure the Helm chart for specific types of dynamic plugin installations.

Configuring a local plugin and an external plugin when the external plugin requires a specific configuration:

[source,yaml]
----
global:
  dynamic:
    plugins:
      - package: <alocal package-spec used by npm pack>
      - package: <external package-spec used by npm pack>
        integrity: sha512-<some hash>
        pluginConfig: ...
----

Deactivating a plugin from an included file:

[source,yaml]
----
global:
  dynamic:
    includes:
      - dynamic-plugins.default.yaml
    plugins:
      - package: <some imported plugins listed in dynamic-plugins.default.yaml>
        enabled: false
----

Activating a plugin from an included file:

[source,yaml]
----
global:
  dynamic:
    includes:
      - dynamic-plugins.default.yaml
    plugins:
      - package: <some imported plugin listed in dynamic-plugins.default.yaml>
        enabled: true
----

Activating a plugin that an included file deactivates:

[source,yaml]
----
global:
  dynamic:
    includes:
      - dynamic-plugins.default.yaml
    plugins:
      - package: <some deactivated plugin listed in dynamic-plugins.default.yaml>
        enabled: true # overrides enabled: false from the included file
----
