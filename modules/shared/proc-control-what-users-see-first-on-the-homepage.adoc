...
[role="_abstract"]
Choose which templates and widgets you see when you log in by configuring the dynamic homepage plugin to display onboarding guides, software templates, entity catalogs, and quick-access cards in customizable or fixed layouts.

The dynamic homepage plugin exposes homepage cards from dynamic plugins and supports both customizable grids where you can drag and resize cards, and read-only grids with fixed positions. Available widgets include onboarding sections, entity catalogs, templates, quick access, search bars, recently visited items, and top visited resources.
