...
. If the permission name includes an `+#action+` suffix, such as `+catalog.entity.create#create+`, the part after the number sign (`+#+`) is passed as the `action` attribute. Without the suffix, the permission is checked with an empty `attributes` object.
...
[NOTE]
====
Permission evaluation occurs during front-end initialization, before the UI renders.
