...
. Optional: Configuration files such as `lightspeed-stack.yaml` and `rhdh-profile.py` are managed by the Helm deployment and overwritten on `Helm upgrade` runs. To protect changes to a file, create a custom config map and reference it in the `values.yaml` file:
+
[source,yaml]
----
intelligentAssistant:
