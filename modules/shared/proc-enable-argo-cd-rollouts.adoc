...
[id="enable-argo-cd-rollouts_{context}"]
= Enable Argo CD Rollouts

[role="_abstract"]
Enable advanced deployment strategies such as blue-green and canary deployments by integrating Argo CD Rollouts with the {product} Kubernetes plugin.
...
When integrated into the backstage Kubernetes plugin, it allows developers and operations teams to visualize and manage Argo CD Rollouts seamlessly within the {product-short} interface.

.Prerequisites

* The {product-short} Kubernetes plugin (`@backstage/plugin-kubernetes`) is installed and configured.

** To install and configure Kubernetes plugin in {product-short}, see link:https://backstage.io/docs/features/kubernetes/installation/[Installation] and link:https://backstage.io/docs/features/kubernetes/configuration/[Configuration] guide.
...
.Procedure
. In the `{my-app-config-file}` file in your {product-short} instance, add the following `customResources` component under the `kubernetes` configuration to enable Argo Rollouts and Analysis Runs:
...
. If the {product-short} Kubernetes plugin is already configured, the `ClusterRole` permissions for Rollouts and AnalysisRuns might already be granted.

. Use the link:https://raw.githubusercontent.com/backstage/community-plugins/main/workspaces/argocd/plugins/argocd/manifests/clusterrole.yaml[prepared manifest] to give read-only `ClusterRole` access to both the Kubernetes and ArgoCD plugins.
====
...
. Add label to Kubernetes resources to enable {product-short} to find the appropriate Kubernetes resources.

.. {product-short} Kubernetes plugin label: Add this label to map resources to specific {product-short} entities.
+
[source,yaml]
----
labels:
...
The label selector will override other annotations such as `kubernetes-id` or `kubernetes-namespace`.
====
