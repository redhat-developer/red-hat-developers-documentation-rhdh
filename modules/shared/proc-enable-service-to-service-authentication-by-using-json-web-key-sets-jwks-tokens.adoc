...
[id="enable-service-to-service-authentication-by-using-json-web-key-sets-jwks-tokens_{context}"]
= Use JSON web key sets (JWKS) tokens
...
The Identity Provider signs the JSON Web Token (JWT) with its private key, then {product-short} verifies it using the public key fetched from the JWKS endpoint.
...
image::control-access_authentication-in-rhdh/jwks-authentication-flow.png["The diagram illustrates the authentication flow between an external service and {product-short}."]
...
* You have configured the external service to obtain a JWT from your Identity Provider and include it in the `Authorization` header of requests to {product-short}.

.Procedure

* Add the JWKS URL to your `{my-app-config-file}` {product-short} configuration file:
+
