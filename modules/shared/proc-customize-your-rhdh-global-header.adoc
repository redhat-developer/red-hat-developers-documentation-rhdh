...
[id="customize-your-rhdh-global-header_{context}"]
= Customize your {product} global header
...
`settings`::
Enter `false` to hide the *Settings* button in the sidebar menu.
Enter `true` to display the *Settings* button in the sidebar menu.
`default.main-menu-items`::
Configures the sidebar menu items. Include the `default.admin` entry to preserve the *Administration* menu item in the sidebar when customizing the global header. Setting `title: ''` for `default.create` hides the *Self-service* button from the sidebar.
`position`::
...
[source,yaml,subs="+attributes,+quotes"]
----
- package: _<npm_or_oci_package_reference>_
  disabled: false
  pluginConfig:
    dynamicPlugins:
      frontend:
...
<npm_or_oci_package_reference>:: Specifies the package name.
