...
product_attributes = root_dir + '/artifacts/attributes.adoc'
asciidoc_modules_dir = root_dir + '/modules/release-notes/'
config_file = asciidoc_modules_dir + '/single-source-release-notes.jira2asciidoc.yml'
...
with open(product_attributes) as file:
  attributes = {}
  for lines in file:
    if re.search(':', lines):
...
product_version_minor_glob = attributes["product-version"] + ".*"
product_version_patch = attributes["product-bundle-version"]

# Load configuration file to get the sections and their Jira queries
