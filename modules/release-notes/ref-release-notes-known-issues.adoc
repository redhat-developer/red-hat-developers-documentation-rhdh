...
== 1.8.5 upgrade for Orchestrator users with Operator deployments requires updating the dynamic-plugins ConfigMap

If you have an existing Operator-backed instance of {product-short} with the Orchestrator, you must update your dynamic-plugins ConfigMap to set the version of the Orchestrator plugins to 1.8.9 once the {product-short} Operator is upgraded to 1.8.5.
Otherwise, the {product-short} instance will not be upgraded at all.

.Example of a dynamic-plugins ConfigMap enabling the Orchestrator plugins in 1.8.9 for Operator-backed instances
[source,yaml]
----
apiVersion: v1
kind: ConfigMap
