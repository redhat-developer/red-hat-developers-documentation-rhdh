...
+
--
Previously, an invalid dynamic plugin file name in ConfigMap prevented the creation of a Backstage CR. With this release, the Backstage CR can be created even with an invalid plugin file name, and a corresponding log message will be displayed if the RHDH build fails.
--

Plugin ID is missing in the permission table in RBAC Administration UI::
+
--
Previously, the plugin ID was missing from the permission table in the *Administration* -> *RBAC*. With this release, updates to both the RBAC backend and frontend plugins now ensure the plugin ID is displayed in the table.
--

Deprecation warning about `backend.auth.keys` in application log::
+
--
Previously, RHDH build logs showed the following deprecation warning:
