...
+
--
When scaling an RHDH instance, role data would become out of sync due to the in-memory cache not being shared between instances.

This issue resulted in inconsistencies in role data across different instances.

With this fix, scaling an RHDH instance will no longer lead to role data being out of sync.
...
+
--
Recent updates to the Gitlab plugin caused organization provider data to not be synchronized.

This issue resulted in the failure to synchronize organization provider data.
