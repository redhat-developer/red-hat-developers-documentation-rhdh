...
{product-very-short} has been enhanced to provide crucial traceability, allowing users to easily identify the template version used to create a specific component. This enhancement helps determine if an older entity needs updating or if a bug is linked to a particular Software Template version.
...
{product} fully supports the `@backstage/plugin-catalog-backend-module-msgraph` plugin, which ingests user and group entities from {azure-brand-name} Active Directory into the software catalog via the Microsoft Graph API.
