...
The {product} Operator provides built-in monitoring capabilities by automatically creating and managing the necessary `ServiceMonitor` resource.

For more information, see {monitoring-and-logging-book-link}#proc-admin-enabling-metrics-ocp-operator_assembly-rhdh-observability[Enabling metrics monitoring in a {product} Operator installation on an {ocp-short} cluster].
...
For more information, see {integrating-with-github-book-link}#importing-multiple-repositories-from-gitlab[Importing multiple GitLab repositories].
...
For more information, see {customizing-book-link}#proc-selecting-rhdh-language_assembly-localization-in-rhdh[Selecting the language for your {product-short} instance].
...
For more information, see {customizing-book-link}#proc-enabling-localization-in-quickstarts_customizing-the-quickstarts[Enabling Quickstart localization in {product-very-short}].
...
For more information, see {customizing-book-link}#localization-support-for-plugins[Localization support for plugins].
...
For more information, see {integrating-with-github-book-link}#bulk-import-scaffolder-template[Input parameters for Bulk Import Scaffolder template].
...
For more information, see {customizing-book-link}#proc-enabling-software-template-version-update-notifications_configuring-templates[Enabling Software Template version update notifications in {product}].
...
For more information, see {customizing-book-link}#customizing-the-home-page[Customizing the Home page].
...
For more information, see {setting-up-and-configuring-your-first-rhdh-instance-link}[{setting-up-and-configuring-your-first-rhdh-instance-title}].
...
For more information, see {installing-and-viewing-plugins-book-link}#assembly-plugin-indicators-and-support-types.adoc_assembly-front-end-plugin-wiring[Utilizing plugin indicators and support types in {product}].
...
For more information, see {configuring-book-link}#HighAvailability[Configuring high availability in {product}].
...
For more information, see {configuring-book-link}#proc-injecting-custom-files-and-environment-variables-into-backstage-containers[Injecting extra files and environment variables into {backstage} containers].