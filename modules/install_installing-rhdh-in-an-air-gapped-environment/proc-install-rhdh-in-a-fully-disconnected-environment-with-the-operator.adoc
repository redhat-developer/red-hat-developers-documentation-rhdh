...
* You have installed GNU `sed`, GNU `tar` 1.35 or later, `jq` 1.7 or later, the {ocp-docs-link}/html-single/disconnected_environments/index#installation-oc-mirror-installing-plugin_about-installing-oc-mirror-v2[`oc-mirror`] tool (recommended on {ocp-short}), the {ocp-docs-link}/html/cli_tools/opm-cli#olm-about-opm_cli-opm-install[`opm`] CLI tool, Podman 5.3 or later, Skopeo 1.20 or later, the `umoci` CLI tool, and `yq` 4.44 or later on your workstation.
