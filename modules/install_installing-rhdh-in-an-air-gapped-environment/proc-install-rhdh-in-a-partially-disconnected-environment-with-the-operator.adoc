...
====
+
. Configure authentication to allow the `kubelet` to pull all {product} images from your mirror registry.
+
For {ocp-short}, update the cluster pull secret:
+
...
====
+
For supported Kubernetes platforms, create a pull secret and add it to the service account:
+
[source,terminal,subs="+quotes"]
----
