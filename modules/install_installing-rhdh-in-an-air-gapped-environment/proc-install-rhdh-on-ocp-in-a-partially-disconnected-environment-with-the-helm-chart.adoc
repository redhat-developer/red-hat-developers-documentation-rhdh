...
--
. Update the cluster pull secret to allow the `kubelet` to pull all {product} images from your mirror registry:
+
[source,terminal,subs="+quotes"]
----
...
helm install _<rhdh_instance>_ _<workspace_directory>_/_<results_directory>_/charts/_<archive_file>_ --namespace _<your_namespace>_ --create-namespace \
  --set openshift.clusterRouterBase="$CLUSTER_ROUTER_BASE"
----
+
--
where:
