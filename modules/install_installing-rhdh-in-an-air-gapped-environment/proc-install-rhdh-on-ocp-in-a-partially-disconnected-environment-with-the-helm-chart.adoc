...
* You have access to a mirror registry that the disconnected cluster can reach, for example, the {ocp-short} image registry. For more information about exposing the {ocp-short} image registry, see {ocp-docs-link}/html-single/registry/index#securing-exposing-registry[Exposing the registry].
* You have logged in to your target mirror registry and have permissions to push images to it. For more information, see {ocp-docs-link}/html-single/disconnected_environments/index#installation-adding-registry-pull-secret_about-installing-oc-mirror-v2[Configuring credentials for mirroring images].
* You have installed GNU `tar` 1.35 or later, `jq` 1.7 or later, the {openshift-cli}, the {ocp-docs-link}/html-single/disconnected_environments/index#installation-oc-mirror-installing-plugin_about-installing-oc-mirror-v2[`oc-mirror`] OpenShift CLI plugin *v2* (recommended), and Skopeo 1.20 or later on your workstation.
