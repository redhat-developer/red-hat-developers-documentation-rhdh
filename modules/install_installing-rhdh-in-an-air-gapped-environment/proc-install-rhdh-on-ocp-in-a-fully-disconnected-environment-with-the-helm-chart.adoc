...
If your network has access to the registry through a bastion host, you can use the Helm chart to install {product}.
...
** You have installed GNU `tar` 1.35 or later, `jq` 1.7 or later, the {openshift-cli}, the `oc-mirror` OpenShift CLI plugin *v2*, and Skopeo 1.20 or later on your workstation.
...
For more information, see the Exposing the registry topic in the {ocp-short} documentation.
...
`<mirror_archive_directory>` :: Enter the location of your directory where the system creates the mirror archive, for example, `pass:[file://.user]`.
--
+
[NOTE]
====
...
[source,terminal,subs="+quotes"]
----
CLUSTER_ROUTER_BASE=$(oc get route console -n openshift-console -o=jsonpath='{.spec.host}' | sed 's/^[^.]*\.//') helm install <rhdh_instance> <workspace_directory>/working-dir/helm/charts/<archive_file> --namespace <your_namespace> --create-namespace \ --set openshift.clusterRouterBase="$CLUSTER_ROUTER_BASE"
----
+
--
where:
