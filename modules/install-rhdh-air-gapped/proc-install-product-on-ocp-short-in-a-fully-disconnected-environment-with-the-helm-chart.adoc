...
[id="install-product-on-ocp-short-in-a-fully-disconnected-environment-with-the-helm-chart_{context}"]
= Install {product} on {ocp-short} in a fully disconnected environment with the Helm chart

[role="_abstract"]
Mirror Helm chart resources to disk and transfer them to a fully disconnected {ocp-short} environment.

If your network has access to the registry through a bastion host, you can use the Helm chart to install {product}. The process mirrors specified resources to disk and transfers them to your air-gapped environment without any connection to the internet.
