...
[id="install-product-on-ocp-short-in-a-partially-disconnected-environment-with-the-helm-chart_{context}"]
= Install {product} on {ocp-short} in a partially disconnected environment with the Helm chart

[role="_abstract"]
Mirror Helm chart resources directly to a target registry in a partially disconnected {ocp-short} environment.

If your network has access to the `registry.redhat.io` registry and the `charts.openshift.io` Helm chart repository, you can deploy your {product} instance in your partially disconnected environment. Mirror the specified resources directly to the target registry.
...
----
$ oc-mirror --config=_<mirror_config_directory>_/ImageSetConfiguration.yaml _<target_mirror_registry>_
----
+
--
where:
...
[NOTE]
====
Running the `oc-mirror` command creates a local workspace that has the Helm chart and a `ImageContentSourcePolicy` (ICSP) manifest. The ICSP manifest has an automatically-generated `imageContentSourcePolicy.yaml` file that you must apply against the cluster in a later step.
====
+
