...
[id="high-availability-with-database-and-cache-layers_{context}"]
= Deploy multiple stateless instances

[role="_abstract"]
Achieving high availability in {product} requires implementing redundancy and failover for both the backend service and its data dependencies.
This is accomplished through horizontal scaling, database replication, and shared caching to ensure continuous operation during component failures.
