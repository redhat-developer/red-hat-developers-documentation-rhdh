...
----
.. Optional: Configure the number of workers that scale the REST API by specifying the following example to the `rcs_config.max_workers` parameter in the {rcs-short} ConfigMap.
+
[source,yaml]
----
rcs_config:
...
[NOTE]
====
Create a dedicated {ls-short} ConfigMap instead of adding an additional section to your existing {product-very-short} custom application configuration file (for example, `custom-app-config.yaml`). Creating two files prevents the entire {product-very-short} ConfigMap from being loaded into {rcs-short}.
====
.. In the {ocp-short} web console, go to your {product-very-short} instance and select the *ConfigMaps* tab.
...
# If set, this value should be in sync with uvicorn_port_number in olsconfig.
...
... In the `spec.application.appConfig.configMaps` section, add the Developer Lightspeed custom app configuration as shown in the following example:
+ 
[source,yaml]
----
...
----
            volumes:
              - configMap:
                ...
                name: rcsconfig
----
... Update the `volumeMounts` specification to mount the RCS ConfigMap as shown in the following example:
+
[source,yaml]
----
        volumeMounts:
...
----
        envFrom:
          - secretRef:
              name: lightspeed-secrets
----
... In your `deployment.patch.spec.template.spec.containers.env` section, add the {rcs-short} image as shown in the following example:
+
[source,yaml]
----
...
====
... Click *Save*. The Pods are automatically updated with the new changes.
+
...
upstream:
 backstage:
   ...
   args:
     ...
   extraAppConfig:
     - configMapRef: lightspeed-config
       filename: lightspeed-app-config.yaml
   extraContainers:
     - env:
         - name: PROJECT
           value: rhdh
