...
* You have installed and activated the `floating-action-button` plugins.
...
`ENABLE_VLLM`:: Set to 'true' for vLLM enablement.
`ENABLE_OPENAI`:: Set to 'true' for Openai enablement.
`ENABLE_VERTEX_AI`:: Set to 'true' for Vertex AI enablement.
`ENABLE_OLLAMA`:: Set to 'true' for Ollama enablement.
[NOTE]
====
If you want to disable them, leave them blank. I.e. `ENABLE_VLLM=""`
====
...
`OPENAI_API_KEY`:: Required for use of OpenAI models via OpenAI API.
`VERTEX_AI_PROJECT`:: Required for use of Gemini through Vertex AI.
`VERTEX_AI_LOCATION`:: Required for use of Gemini through Vertex AI.
`GOOGLE_APPLICATION_CREDENTIALS`:: Required for use of Gemini through Vertex AI.
`SAFETY_MODEL`:: Recommended value is `llama-guard3:8b`
`SAFETY_URL`:: The URL that hosts the safety model. This is post-fixed with `/v1`. For example: `localhost:#####/v1`
`SAFETY_API_KEY`: The API key for that model if applicable.
...
plugins:
- disabled: false
  package: oci://ghcr.io/redhat-developer/rhdh-plugin-export-overlays/red-hat-developer-hub-backstage-plugin-lightspeed:bs_1.45.3__1.2.2
  pluginConfig:
    dynamicPlugins:
      frontend:
...
. Required if your environment uses another global floating action button (FAB): You must move the existing FAB or disable it to prevent interface elements from overlapping by updating your dynamic plugin configuration file with the following changes:
+
[source,yaml]
----
...
containers:
  ...
  - envFrom:
      - secretRef:
          name: llama-stack-secrets
    image: 'quay.io/redhat-ai-dev/llama-stack:0.1.3' # Llama Stack image
    name: llama-stack
    volumeMounts:
