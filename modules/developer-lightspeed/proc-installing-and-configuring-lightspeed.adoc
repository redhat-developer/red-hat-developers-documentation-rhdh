...
** {installing-on-ocp-book-url}#assembly-install-rhdh-ocp-operator[Using the Operator]
** {installing-on-ocp-book-url}#assembly-install-rhdh-ocp-helm[Using the Helm chart]
  ...
  application:
    extraFiles:
      ...
      configMaps:
        - name: rbac-policies
----
For detailed information, see {authorization-book-url}managing-authorizations-by-using-external-files[Managing authorizations by using external files].
