...
To declare users such as _<your_policy_administrator_name>_ as policy administrators, edit your custom {product-very-short} ConfigMap, such as `{my-app-config-file}`, and add following code to the `app-config.yaml` content:
+
[source,yaml]
----
permission:
