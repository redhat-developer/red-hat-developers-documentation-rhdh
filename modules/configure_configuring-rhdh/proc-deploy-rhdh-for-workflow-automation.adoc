...
[IMPORTANT]
====
If you have a custom `dynamic-plugins` ConfigMap defined using `spec.application.dynamicPluginsConfigMapName`, the custom configuration takes precedence over the Orchestrator flavour configuration.
To enable the Orchestrator flavour, remove Orchestrator plugins from your custom `dynamic-plugins` ConfigMap.
To disable the Orchestrator flavour, set `spec.flavours: [{name: orchestrator, enabled: false}]` and verify that no Orchestrator plugins are defined in your custom `dynamic-plugins` ConfigMap.
If Orchestrator plugins remain in your custom ConfigMap, they will be loaded regardless of the flavour setting.
====
