...
[role="_abstract"]
You can configure the {product-short} route to use a TLS certificate stored in a Kubernetes secret by setting the `externalCertificateSecretName` field in your `{product-custom-resource-type}` custom resource.
