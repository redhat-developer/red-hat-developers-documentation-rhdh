...
Both files are provisioned together in a single config map, such as, `rbac-policies`. Reference the files from `{my-app-config-file}` by using the following fields:
