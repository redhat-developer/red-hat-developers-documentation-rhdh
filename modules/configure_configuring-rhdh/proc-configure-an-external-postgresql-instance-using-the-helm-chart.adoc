...
`postgresql.enabled`:: Set to `false` to disable the local PostgreSQL instance creation.
`postgresql.auth.existingSecret`:: The credentials secret to inject into {product-custom-resource-type}.
`appConfig.backend.database.connection`:: The {product-custom-resource-type} database connection parameters.
`extraEnvFrom`:: The credentials secret to inject as environment variables into {product-custom-resource-type}.
