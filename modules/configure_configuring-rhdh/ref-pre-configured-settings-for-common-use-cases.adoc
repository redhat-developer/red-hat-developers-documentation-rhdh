...
[IMPORTANT]
====
Custom `dynamic-plugins` ConfigMap settings take precedence over flavour configurations.
If you define a custom `dynamic-plugins` ConfigMap using `spec.application.dynamicPluginsConfigMapName`, plugins in the custom ConfigMap override flavour-provided plugins.
Enabling or disabling a flavour does not affect plugins explicitly defined in your custom ConfigMap.
To use flavour configurations, remove the relevant plugins from your custom `dynamic-plugins` ConfigMap.
====
