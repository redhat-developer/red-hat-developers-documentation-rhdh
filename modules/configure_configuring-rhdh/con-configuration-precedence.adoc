...
When you provide multiple `{my-app-config-file}` files (for example, through multiple ConfigMaps), {product-short} deep-merges them. Later files override earlier files for the same keys. For object values, the merge is recursive. For array values, the later array replaces the earlier array entirely.
