...
. For each repository to import, click on the *PR* link to review and merge the changes in Gitlab.

.Verification
. Click *Bulk Import* in {product-short} left sidebar.
. In the *Selected repositories* list, each imported GitLab project has the appropriate status: either _Waiting for approval_ or _Added_.
