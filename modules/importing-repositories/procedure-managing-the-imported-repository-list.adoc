...
* A location added through other sources (like statically in an `{my-app-config-file}` file, dynamically when link:{integrating-with-github-book-url}#enabling-github-repository-discovery[enabling GitHub discovery], or registered manually using the "Register an existing component" page) might show up in the Bulk Import list of Added Repositories if the following conditions are met:
