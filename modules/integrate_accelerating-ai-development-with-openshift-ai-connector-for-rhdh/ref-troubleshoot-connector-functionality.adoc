...
If model cards do not appear as TechDocs, confirm that the `InferenceService` or `LLMInferenceService` resources are annotated with `rhdh.io/catalog-source` and `rhdh.io/catalog-model`, and that the Kubeflow Model Catalog URL is reachable from the {product-very-short} backend.
