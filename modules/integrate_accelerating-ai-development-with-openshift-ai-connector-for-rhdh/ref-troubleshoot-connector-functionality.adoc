...
The connector mounts its REST routes under the `/api/kserve-kubeflow-connector/` path prefix in the {product-very-short} backend. To call a route directly with `curl` or to find it in the backend logs, prepend `https://<rhdh-host>/api/kserve-kubeflow-connector/` to the route path.
...
If model cards do not appear as TechDocs, confirm that the `InferenceService` or `LLMInferenceService` resources are annotated with `rhdh.io/catalog-source` and `rhdh.io/catalog-model`, and that the Kubeflow Model Catalog URL is rWhen the connector communicates with the cluster API server with TLS verification enabled, it needs the certificate authority (CA) certificate that signed the API server external serving certificate. This CA is not necessarily the one that is mounted inside pods for internal cluster communication.eachable from the {product-very-short} backend.

== Query the Kubeflow Model Catalog

To access the same Kubeflow Model Catalog data as the connector, use `curl` to query the model catalog API directly. Confirm that the `ServiceAccount` token has the correct access, and export the model catalog URL as `{rhoai-short}_MODEL_CATALOG_URL` before you run the following commands.
...
[source,bash,subs="+attributes,+quotes"]
----
$ curl -k -H "Authorization: Bearer $TOKEN" ${rhoai-short}_MODEL_CATALOG_URL/api/model_catalog/v1alpha1/models?pageSize=200 | jq
----
+
[NOTE]
====
