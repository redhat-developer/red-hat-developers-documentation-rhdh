...
|TechDocs
|N/A
|Associates a model card from the Kubeflow Model Catalog with the generated entity as {product-very-short} TechDocs. Model cards are imported when the `InferenceService` or `LLMInferenceService` is annotated to reference a catalog source and model.
|===
