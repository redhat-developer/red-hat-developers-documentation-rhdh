. Mirror the images for the Serverless Operator {rhoserverless-version} and the Serverless Logic Operator {rhoserverless-version} using tools like Skopeo or podman.
+
...
----
+
Use the latest version of the bundle operator, {rhoserverless-version} to avoid missing or having outdated image references. For more information, refer to {orchestrator-book-link}#con-compatibility-guide-for-orchestrator.adoc_orchestrator-rhdh[Compatibility guide for Orchestrator].
