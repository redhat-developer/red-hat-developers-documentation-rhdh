...
In a fully network-restricted environment, a disconnected installation prevents unauthorized access, data transfer, or communication with external sources.
...
====
+
. Transfer the directory specified by `/path/to/mirror/dir/on/disk` to a host within your disconnected environment.

. From the host which has access to the mirror registry, mirror the images from the disk directory to your target registry. For example:
+
[source,subs="+attributes,+quotes"]
----
