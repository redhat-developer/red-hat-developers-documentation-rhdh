...
* You have mirrored the {product} Operator images to the local registry using the {product-very-short} mirroring script. For more information, see {installing-in-air-gap-book-link}#proc-install-rhdh-operator-airgapped-partial_title-install-rhdh-air-grapped[Installing {product} in a partially disconnected environment with the Operator].
