...
+
The Orchestrator Helm chart uses the Helm `lookup` function to check for an existing PostgreSQL secret. Because Argo CD renders manifests using `helm template`, it cannot query the live cluster. This causes the chart to generate a new, random password instead of retrieving the existing one, resulting in an authentication failure.
+
Therefore, complete the following steps:

.. Manually create the database secret with the correct credentials:
+
