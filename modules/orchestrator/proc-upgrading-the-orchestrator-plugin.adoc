...
You can upgrade an existing `1.7` Operator-backed instance with Orchestrator enabled by upgrading the {product} Operator to {product-version}. After upgrading the Operator to {product-version}, manually update the `dynamic-plugins` ConfigMap to set the Orchestrator plugin versions to `1.8.9`.
...
. Update your `dynamic-plugins` ConfigMap to set the version of the Orchestrator plugin to `1.8.9`.
+
