...
= Updating the Orchestrator plugin to support the {product-short} {product-bundle-version} upgrade

You can upgrade an Operator-backed instance with Orchestrator enabled by upgrading the {product} Operator to {product-bundle-version}. After upgrading the Operator, manually update the `dynamic-plugins` ConfigMap to set the Orchestrator plugin versions to {orchestrator-plugin-version}.

.Prerequisites
* You have have a running instance of {product} with Orchestrator 1.7 or 1.8.2 backed by the Operator.
