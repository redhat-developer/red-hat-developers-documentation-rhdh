...
[id="orchestrator-plugin-dependencies-for-operator-installation_{context}"]
= Orchestrator plugin dependencies for Operator installation

[role="_abstract"]
When you enable the Orchestrator plugin in your {product-custom-resource-type} custom resource (CR), the Operator automatically provisions required dependencies.
