...
[NOTE]
====
In {product-very-short} {rhoserverless-version}, images for the Serverless Logic Operator (such as PostgreSQL and job services) are automatically resolved by the Operator bundle and do not need to be manually listed in the `additionalimages` section.
====
+
