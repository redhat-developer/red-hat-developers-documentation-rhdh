...
[id="proc-install-rhdh-orchestrator-airgapped-env-using-helm-chart-full.adoc_{context}"]
...
In a fully network-restricted environment, you can mirror images to an intermediary disk, and then mirror from the disk to your target registry.
...
. Create an `ImageSetConfiguration` file for `oc-mirror`. You must use an `ImageSetConfiguration` file to include all mirrored images required by the Serverless Logic Operator as shown in the following example:
+
...
====
+
. Transfer the generated mirror archive file, for example, `/path/to/mirror-archive/mirror_seq1_000000.tar`, to a host within your disconnected environment.

. From the host in your disconnected environment, which has access to the mirror registry, mirror the images from the archive file to your target registry. For example:
+
[source,subs="+attributes,+quotes"]
----
