...
The Orchestrator plugin uses specific permission strings to control access to workflows and instances. When you enable the RBAC plugin, the Orchestrator UI does not list workflows until you grant these permissions in your `rbac-policy.csv` file:

`orchestrator.workflow` (read)::
Required to list and view workflow definitions and their instances. If this permission is missing, the workflow list in the UI appears empty.
`orchestrator.workflow.use` (update)::
Required to execute (run) or abort a workflow.
`orchestrator.workflowAdminView` (read)::
Grants access to the workflow definition editor and instance variables.
`orchestrator.instanceAdminView` (read)::
Allows the user to view all workflow instances, including those created by other users.
