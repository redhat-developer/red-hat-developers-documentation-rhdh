...
By using the `spec.monitoring.enabled` field to `true` in your {product} custom resource (CR), you instruct the Operator to automatically create and manage the necessary `ServiceMonitor` to scrape metrics from the service endpoint.
...
[NOTE]
====
If custom labels were required for the manual `ServiceMonitor` (specifically, ensuring `spec.selector.matchLabels` matched the {product-very-short} installation labels), make sure the Operator provides appropriate fields within `spec.monitoring.enabled` to accommodate these requirements.
====
