...
By using the `spec.application.serviceMonitor` field in your {product} custom resource (CR), you instruct the Operator to automatically create and manage the necessary `ServiceMonitor` to scrape metrics from the service endpoint.
...
[source,bash]
----
oc edit DeveloperHub <instance-name>
----
. Within the CR definition, locate the `spec.application` field and add the `serviceMonitor` configuration block. This field instructs the Operator to create the `ServiceMonitor`` resource necessary to scrape metrics automatically.
+
[source,yaml]
----
spec:
...
[NOTE]
====
If custom labels were required for the manual `ServiceMonitor` (specifically, ensuring `spec.selector.matchLabels` matched the {product-very-short} installation labels), make sure the Operator provides appropriate fields within `spec.application.serviceMonitor` to accommodate these requirements.
====
