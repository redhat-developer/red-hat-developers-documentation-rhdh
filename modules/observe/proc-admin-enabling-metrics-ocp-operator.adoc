...
----
. Within the CR definition, locate the `spec` field and add the `monitoring` configuration block. This field instructs the Operator to create the `ServiceMonitor` resource necessary to scrape metrics automatically.
+
[source,yaml]
----
spec:
...
[NOTE]
====
The Operator automatically configures the `ServiceMonitor` with the correct labels (`app.kubernetes.io/instance` and `app.kubernetes.io/name`) that match your Backstage installation. The `ServiceMonitor` will be named `metrics-<cr-name>`. No additional label configuration is required.
====
