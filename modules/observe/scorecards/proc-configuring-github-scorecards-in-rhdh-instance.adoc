...
To enable the GitHub metrics integration, you must create and configure a GitHub App to grant {product-very-short} access to the GitHub API using either a https://docs.github.com/en/apps/overview[GitHub App] or a GitHub token.
+
[IMPORTANT]
====
For accessing resources on behalf of an organization, or for long-lived integrations, you should use a GitHub App 
====
...
. To allow {product-very-short} to access the GitHub API, you must set up authentication (by creating and configuring a GitHub App or generating a GitHub token) and add the necessary integration to your configuration file:
+
[NOTE]
====
For GitHub App installation, you must install the GitHub App on the organization (or user account) that owns repositories, granting it the necessary repository access permissions.
====
