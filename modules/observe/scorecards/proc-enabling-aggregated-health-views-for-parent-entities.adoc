    ...
    scorecard:
      aggregation:
        ...
        kinds:
          ...
          - Domain
----
. Save the changes to {product-very-short} `{my-app-config-file}`.
. Restart your {product-very-short} instance to apply the configuration.
. To enable aggregation for a specific entity, you must update that entity's `catalog-info.yaml` to include the `scorecard.redhat.com/aggregation` annotation:
+
[source,yaml]
----
