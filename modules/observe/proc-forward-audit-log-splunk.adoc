...
[role="_abstract"]
Forward audit logs to Splunk by using the Logging Operator and ClusterLogForwarder to capture streamed logs and send them to your Splunk instance.
...
You can either use `tls.insecureSkipVerify` option if the Splunk endpoint uses self-signed TLS certificates (not recommended) or provide the certificate chain using a Secret.
+
[source,yaml]
----
outputs:
...
Use the names of the defined inputs and outputs to specify multiple `inputRefs` and `outputRefs` in each pipeline:
+
...
+
`AtLeastOnce`::
`AtLeastOnce` delivery mode means that if the log forwarder crashes or is restarted, any logs that were read before the crash but not sent to their destination are re-sent.
