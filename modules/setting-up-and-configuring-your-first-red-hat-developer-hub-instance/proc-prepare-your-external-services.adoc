...
[id="prepare-your-external-services_{context}"]
= Prepare your external services

[role="_abstract"]
{product} relies on external services for production use, including a PostgreSQL database, Redis cache, GitHub API access, and an identity provider.
...
Redis cache::
For efficiency, {product-short} caches plugin and TechDocs assets when you provide a Redis cache server.
