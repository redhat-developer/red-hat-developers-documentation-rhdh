...
The Quickstart plugin provides guided onboarding for users of {product}. It displays a customizable drawer interface with interactive quickstart steps that help users get familiar with the platform.

[NOTE]
====
If RBAC is not enabled, Quickstart is only accessible to users with administrator permissions.
====
