...
. Reconfigure your `Backstage` custom resource (CR). For more information, see link:{LinkAdminGuide}#proc-configuring-postgresql-instance-using-operator_admin-rhdh[Configuring an external PostgreSQL instance using the Operator] or link:{LinkAdminGuide}#proc-configuring-postgresql-with-tls-using-operator_admin-rhdh[Configuring an external PostgreSQL instance with TLS connection using the Operator]. 
