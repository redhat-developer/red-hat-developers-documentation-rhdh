...
There are two types of permissions, resource type and basic named permissions, within {product} that can be defined. The difference between the two is whether or not a permission has a resource type defined.

Resource type permissions can be defined using either the resource type of that particular permission or their name.

Example of a resource type permission where we define the ability to read from the catalog using the name of the permission for one role and the resource type for another:
...
Basic named permissions on the other hand are only able to be defined using their name.

Example of a basic named permission for the ability to create catalog entities.
