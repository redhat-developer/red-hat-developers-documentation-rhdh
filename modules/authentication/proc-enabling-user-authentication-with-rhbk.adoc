...
Authenticate users with {rhbk-brand-name} ({rhbk}),
by provisioning the users and groups from {rhbk} to the {product-short} software catalog, and configuring the OpenID Connect (OIDC) authentication provider in {product}.
