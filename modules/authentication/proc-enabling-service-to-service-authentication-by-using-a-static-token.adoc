...
This method is simpler to set up compared to using JWKS tokens, but it requires careful management of the static token to ensure security.
...
Documentation::
Document all authentication methods in use and maintain an inventory of all tokens and their purposes, and keep security policies up to date.
...
However, it's important to note that static token authentication might not be suitable for:
...
----
+
This command generates a 24-byte random value and encodes it in base64 format, resulting in a token that is sufficiently strong for authentication purposes, and properly encoded for use in HTTP headers.
...
----
+
Replace the following placeholders in the configuration snippet above:

token:: Enter the environment variable name from the previous step.
...
* In the Audit Logs of the service receiving the request, verify that the request was authenticated successfully using the `subject` value as  the actor.
