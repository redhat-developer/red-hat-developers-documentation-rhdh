...
.. On the *Home > App registrations > __<My {product-short}>__ > Manage > Authentication* page, *Add a platform*, with the configuration:
...
.. On the *Home > App registrations > __<My {product-short}>__ > Manage > API permissions* page, *Add a Permission*,
then add the following *Delegated permission* for the
*Microsoft Graph API*:
+
* `email`
* `offline_access`
