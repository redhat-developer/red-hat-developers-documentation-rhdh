...
Additionally, you can set the `NO_PROXY` environment variable to exclude certain domains from proxying. The variable value is a list of domains or IP addresses that do not require a proxy to get reached, even if one is specified.
