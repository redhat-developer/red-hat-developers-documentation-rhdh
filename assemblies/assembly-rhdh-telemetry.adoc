...
With its default configuration, {product} sends telemetry data to Red Hat by using the `backstage-plugin-analytics-provider-segment` plugin. 

[IMPORTANT]
====
To disable the telemetry data collection feature, see the link:{LinkAdminGuide}#disabling-telemetry-data-collection_admin-rhdh[Disabling telemetry data collection in {product-very-short}] section.
====
