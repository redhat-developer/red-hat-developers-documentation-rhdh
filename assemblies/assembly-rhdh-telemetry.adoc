...
The telemetry data collection feature is enabled by default. With its default configuration, {product} sends telemetry data to Red Hat by using the `backstage-plugin-analytics-provider-segment` plugin. 

[IMPORTANT]
====
To disable the telemetry data collection feature, see the link:{LinkAdminGuide}#disabling-telemetry-data-collection_admin-rhdh[Disabling telemetry data collection in {product-very-short}] section. In an air-gapped environment, you can disable this feature to avoid needless outbound requests affecting the responsiveness of the {product-very-short} application.
====
