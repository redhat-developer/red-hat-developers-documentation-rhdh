...
[role="_additional-resources"]
.Additional resources
* xref:assembly-auth-provider-github_proc-configuring-github-app[Configuring a GitHub App in {product-short}]
* xref:assembly-enabling-authentication_proc-adding-gitlab-oauth-as-an-authentication-provider[Enabling the GitLab OAuth authentication provider]
