...
To benefit from over-the-air updates and catalogs provided by Operator-based applications distributed with the Operator Lifecycle Manager (OLM) framework, consider installing {product} by using the {product} Operator distributed in the {rhec}.

On {platform}, the most notable differences over an OpenShift-based installation are:

* The OLM framework and the {rhec} are not built-in.
* The {rhec} pull-secret is not managed globally.
