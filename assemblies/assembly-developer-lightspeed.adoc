:_mod-docs-content-type: ASSEMBLY
:context: developer-lightspeed

[id="{context}"]
= Interacting with {ls-brand-name}

[role="_abstract"]
{ls-brand-name} ({ls-short}) is a virtual assistant powered by generative Artificial Intelligence (AI) designed for {product}({product-very-short}). The assistant offers in-depth insights into {product-very-short}, including its wide range of capabilities. You can interact with this assistant to explore and learn more about {product-very-short} in greater detail.

include::modules/developer-lightspeed/con-about-developer-lightspeed.adoc[leveloffset=+1]

include::modules/developer-lightspeed/con-supported-architecture.adoc[leveloffset=+1]

include::modules/developer-lightspeed/con-about-lightspeed-stack-and-llama-stack.adoc[leveloffset=+2]

include::modules/developer-lightspeed/con-rag-embeddings.adoc[leveloffset=+1]

include::modules/developer-lightspeed/proc-install-and-configure-lightspeed.adoc[leveloffset=+1]

include::assembly-customizing-developer-lightspeed.adoc[leveloffset=+1]

include::assembly-get-ai-assisted-help-for-your-development-tasks.adoc[leveloffset=+1]

include::assembly-appendix-llm-requirements.adoc[leveloffset=+1]

include::assembly-appendix-about-user-data-security.adoc[leveloffset=+1]
