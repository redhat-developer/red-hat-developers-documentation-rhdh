:_mod-docs-content-type: ASSEMBLY
:context: assembly-using-techdocs
[id="{context}"]
= Using TechDocs

The TechDocs plugin is installed and enabled on your {product} instance by default. After an administrator configures the TechDocs plugin, a developer can use TechDocs to view and manage the imported documentation.

include::assembly-techdocs-add-docs.adoc[leveloffset=+1]

include::modules/techdocs/proc-techdocs-find-docs.adoc[]

include::modules/techdocs/proc-techdocs-view-docs.adoc[]

include::modules/techdocs/proc-techdocs-edit-docs.adoc[]
