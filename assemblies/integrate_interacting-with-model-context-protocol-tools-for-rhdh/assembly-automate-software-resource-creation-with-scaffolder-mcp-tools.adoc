...
These tools enable AI agents to understand and involve Scaffolder capabilities through natural language input within an IDE or chat interface. You can validate template logic using sandboxed dry-runs and monitor the progress of automated tasks. To maintain security, these tools must adhere to existing role-based access control (RBAC) permissions and maintain a consistent audit trail.
...
include::../modules/integrate_interacting-with-model-context-protocol-tools-for-rhdh/ref-monitor-scaffolder-tasks-with-mcp-tools.adoc[leveloffset=+1]
