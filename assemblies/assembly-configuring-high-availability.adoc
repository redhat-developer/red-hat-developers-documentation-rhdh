...
You can configure High Availability in {product-short} by scaling your `replicas` {user-home} to a number greater than 1 in your configuration file. The configuration file that you use depends on the method that you used to install your {product-short} instance. If you used the Operator to install your {product-short} instance, configure the replica values in your `{product-custom-resource-type}` CR. If you used the Helm chart to install your {product-short} instance, configure the replica values in your Helm chart.
